...
if menu == "Registrar nueva factura":
    ...
    if guardar:
        if num_fact in df_facturas["No. Factura"].values:
            ...
            nueva_factura = pd.DataFrame([[fecha, cliente, num_fact, importe, notas]], columns=df_facturas.columns)
    ...
    if registrar:
        nuevo_pago = pd.DataFrame([[factura, fecha_pago, importe_pagado, metodo]], columns=df_pagos.columns)
    ...
    if clientes:
        ...
        resumen["Pagado"] = resumen["No. Factura"].apply(lambda x: pagos_cliente[pagos_cliente["No. Factura"] == x]["Importe Pagado"].sum())
    ...
    resumen["Pagado"] = resumen["No. Factura"].apply(lambda x: df_pagos[df_pagos["No. Factura"] == x]["Importe Pagado"].sum())
    resumen["Saldo"] = resumen["Importe"] - resumen["Pagado"]
    ...
    totales = pd.DataFrame({
        "Fecha": [""], "Cliente": [""], "No. Factura": ["TOTAL"],
        "Importe": [tabla["Importe"].sum()], "Saldo": [tabla["Saldo"].sum()],
        "Al día": [tabla["Al día"].sum()], "1 a 30": [tabla["1 a 30"].sum()],
        "31 a 60": [tabla["31 a 60"].sum()], "61 a 90": [tabla["61 a 90"].sum()],
    ...
    tabla_para_exportar = pd.concat([tabla, totales], ignore_index=True)
    tabla_para_exportar.to_excel(export_buffer, index=False, sheet_name="Antigüedad")
    export_buffer.seek(0)
    st.download_button(
        label="📥 Descargar reporte en Excel",
        data=export_buffer,
        file_name="reporte_antiguedad.xlsx",
